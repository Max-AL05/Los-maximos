"""
Endpoints:
    GET /reportes/calificaciones/<materia_id>?formato=pdf|xls
    GET /reportes/asistencias/<materia_id>?formato=pdf|xls
"""
...
def _serve(tipo, materia_id, formato, user_id):
    ...
    ReporteGenerado.objects.create(
        ...
        generado_por_user_id=str(user_id) if user_id else "",
        file_name=filename,
...
def reporte_calificaciones(request, materia_id):
    ...
    return _serve("CALIFICACIONES", materia_id, formato, request.user.id)
...
def reporte_asistencias(request, materia_id):
    formato = request.query_params.get("formato", "pdf")
    return _serve("ASISTENCIAS", materia_id, formato, request.user.id)
