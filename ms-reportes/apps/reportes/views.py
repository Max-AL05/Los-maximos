...
def _serve(tipo, materia_id, formato, user_id):
    ...
    # Publicar evento asincrono: MS-6 notifica al docente que su reporte esta listo
    try:
        from shared.rabbitmq.publisher import publish_event
        from shared.rabbitmq.events import REPORTE_GENERADO
        publish_event(REPORTE_GENERADO, {
            "reporte_id":     str(reporte.id) if hasattr(reporte, "id") else "",
            "tipo":           tipo,
            "formato":        formato,
            "materia_id":     str(materia_id),
            "materia_nombre": "",  # MS-6 puede resolver via gRPC GetMateriaById
            "docente_id":     str(user_id) if user_id else "",
            "docente_email":  "",  # MS-6 resuelve via gRPC GetUserById
            "docente_nombre": "",
            "file_name":      filename,
        })
    except Exception:
        pass  # La descarga del reporte no falla aunque RabbitMQ no este disponible
