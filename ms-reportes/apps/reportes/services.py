...
# Colores institucionales aproximados a la hoja de FCC-BUAP
AZUL_BUAP = "00637C"   # azul cyan oscuro del header del directorio
GRIS_CLARO = "EAEAEA"
NEGRO = "000000"
...
def generar_calificaciones_xlsx(materia_id) -> tuple[bytes, str, str]:
    ...
    # Tabla
    headers = ["#", "Matrícula", "Nombre completo", "Promedio real",
               "Redondeado", "Estado"]
    ...
    for idx, alu in enumerate(alumnos, start=1):
        ...
        for col, v in enumerate(row_data, start=1):
            ...
            _apply_cell_style(c, align=align)
            if col == 6:  # color por estado
                if v == "APROBADO":
    ...
    return (
        buf.getvalue(),
        f"calificaciones_{materia.get('nrc') or materia_id}.xlsx",
        XLSX_MIME,
    )
...
def generar_calificaciones_pdf(materia_id) -> tuple[bytes, str, str]:
    ...
    title_st = ParagraphStyle(
        "title", parent=styles["Title"], fontSize=14, textColor=colors.HexColor(f"#{AZUL_BUAP}"),
    ...
    story = []
    story.append(Paragraph("BENEMÉRITA UNIVERSIDAD AUTÓNOMA DE PUEBLA", title_st))
    story.append(Paragraph("Facultad de Ciencias de la Computación", sub_st))
    ...
    # Datos de la materia (tabla 2 columnas)
    info_data = [
    ...
    tbl = Table(rows, colWidths=[0.8 * cm, 2.4 * cm, 7.0 * cm, 2.0 * cm, 1.6 * cm, 2.4 * cm],
                repeatRows=1)
    style_cmds = [
    ...
    # Resumen
    resumen_data = [
        ["Total alumnos:", str(stats["total_alumnos"]),
         "Promedio grupal:", f"{stats['promedio_grupal']:.2f}"],
        ["Aprobados:", str(stats["aprobados"]),
         "Calif. máxima:", f"{stats['calificacion_maxima']:.2f}"],
        ["Reprobados:", str(stats["reprobados"]),
         "Calif. mínima:", f"{stats['calificacion_minima']:.2f}"],
    ]
    ...
    resumen_tbl.setStyle(TableStyle([
        ...
        ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor(f"#{GRIS_CLARO}")),
        ("BACKGROUND", (0, 1), (-1, 1), colors.HexColor(f"#{GRIS_CLARO}")),
        ("BACKGROUND", (0, 2), (-1, 2), colors.HexColor(f"#{GRIS_CLARO}")),
        ("LEFTPADDING", (0, 0), (-1, -1), 6),
        ("RIGHTPADDING", (0, 0), (-1, -1), 6),
    ]))
    story.append(Paragraph("<b>Resumen del grupo</b>", label_st))
    ...
    return (
        buf.getvalue(),
        f"calificaciones_{materia.get('nrc') or materia_id}.pdf",
        PDF_MIME,
    )
...
def generar_asistencias_xlsx(materia_id) -> tuple[bytes, str, str]:
    ...
    total_sesiones = stats.get("total_sesiones", 0) or 1  # evitar div/0
    ...
    for idx, alu in enumerate(alumnos, start=1):
        ...
        # Si no devuelve ausencias explícitas, las inferimos de las sesiones totales
        if aus == 0 and total_sesiones > (pres + ret):
            aus = total_sesiones - pres - ret
        pct = round((pres + ret) / total_sesiones * 100, 1) if total_sesiones else 0
    ...
    return (
        buf.getvalue(),
        f"asistencias_{materia.get('nrc') or materia_id}.xlsx",
        XLSX_MIME,
    )
...
def generar_asistencias_pdf(materia_id) -> tuple[bytes, str, str]:
    ...
    story = [
        ...
        Paragraph(
            f"<b>Concentrado de Asistencias</b> &nbsp;&nbsp; "
            f"Generado: {datetime.now():%Y-%m-%d %H:%M}",
            sub_st,
        ),
    ]
    ...
    tbl = Table(rows,
                colWidths=[0.8 * cm, 2.4 * cm, 6.4 * cm, 1.6 * cm, 1.6 * cm, 1.6 * cm, 1.8 * cm],
                repeatRows=1)
    ...
    resumen_data = [
        ["Total sesiones:", str(stats.get("total_sesiones", 0)),
         "% Asist. grupal:", f"{stats.get('porcentaje_asistencia', 0)}%"],
        ["Asistencias:", str(stats.get("total_asistencias", 0)),
         "Retardos:", str(stats.get("total_retardos", 0))],
    ...
    return (
        buf.getvalue(),
        f"asistencias_{materia.get('nrc') or materia_id}.pdf",
        PDF_MIME,
    )


# =====================================================================
# Registro: lo usa la vista REST (y el servicer gRPC) para resolver
#           el generador correcto a partir de (tipo, formato).
# =====================================================================
REGISTRO = {
    ("CALIFICACIONES", "XLSX"): generar_calificaciones_xlsx,
    ("CALIFICACIONES", "XLS"): generar_calificaciones_xlsx,  # mismo file, alias
