...
def _enviar_bienvenida_ms6(alumno_id: str, materia_id: str, clave: str,
                           alumno_email: str = "", alumno_nombre: str = "",
                           materia_nombre: str = ""):
    """
    Publica evento 'alumno.registrado' en RabbitMQ.
    MS-6 Notificaciones consume la cola y envia el correo de bienvenida.
    Comunicacion ASINCRONA: si MS-6 cae, el mensaje queda en la cola.
    """
    try:
        from shared.rabbitmq.publisher import publish_event
        from shared.rabbitmq.events import ALUMNO_REGISTRADO
        publish_event(ALUMNO_REGISTRADO, {
        ...
        pass  # El registro del alumno no falla aunque RabbitMQ no este disponible


def _enviar_baja_ms6(alumno_id: str, docente_id: str, materia_id: str,
                     alumno_nombre: str = "", materia_nombre: str = "",
                     docente_email: str = "", docente_nombre: str = ""):
    """
    Publica evento 'alumno.baja' en RabbitMQ.
    MS-6 Notificaciones consume la cola y avisa al docente.
    Comunicacion ASINCRONA: la baja se persiste aunque MS-6 este caido.
    """
    try:
        from shared.rabbitmq.publisher import publish_event
        from shared.rabbitmq.events import ALUMNO_BAJA
        publish_event(ALUMNO_BAJA, {
            "alumno_id":      str(alumno_id),
            "alumno_nombre":  alumno_nombre,
            "materia_id":     str(materia_id),
            "materia_nombre": materia_nombre,
